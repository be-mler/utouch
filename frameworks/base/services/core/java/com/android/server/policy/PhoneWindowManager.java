...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    /** U-Touch right swipe */
    private int mFpRightSwipe;
    /** U-Touch left swipe */
    private int mFpLeftSwipe;
    ...
    private void updateKeyAssignments() {
        ...
        if (mLongPressOnHomeBehavior < KEY_ACTION_NOTHING ||
                mLongPressOnHomeBehavior > KEY_ACTION_LAST_APP) {
        ...
        if (mDoubleTapOnHomeBehavior < KEY_ACTION_NOTHING ||
                mDoubleTapOnHomeBehavior > KEY_ACTION_LAST_APP) {
            mDoubleTapOnHomeBehavior = KEY_ACTION_NOTHING;
        }

        /** U-Touch right swipe */
        mFpRightSwipe = mContext.getResources().getInteger(
                com.android.internal.R.integer.config_fpRightSwipe);
        if (mFpRightSwipe < KEY_ACTION_NOTHING ||
                mFpRightSwipe > KEY_ACTION_LAST_APP) {
            mFpRightSwipe = KEY_ACTION_NOTHING;
        }
        /** U-Touch left swipe */
        mFpLeftSwipe = mContext.getResources().getInteger(
                com.android.internal.R.integer.config_fpLeftSwipe);
        if (mFpLeftSwipe < KEY_ACTION_NOTHING ||
                mFpLeftSwipe > KEY_ACTION_LAST_APP) {
            mFpLeftSwipe = KEY_ACTION_NOTHING;
        }
        ...
        if (hasHome) {
            ...
            mFpRightSwipe = CMSettings.System.getIntForUser(resolver,
                    CMSettings.System.KEY_FP_RIGHT_SWIPE_ACTION,
                    mFpRightSwipe, UserHandle.USER_CURRENT);

            mFpLeftSwipe = CMSettings.System.getIntForUser(resolver,
                    CMSettings.System.KEY_FP_LEFT_SWIPE_ACTION,
                    mFpLeftSwipe, UserHandle.USER_CURRENT);
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (keyCode == KeyEvent.KEYCODE_FP_RIGHT_SWIPE && down){
            performKeyAction(mFpRightSwipe, event);
        }

        if (keyCode == KeyEvent.KEYCODE_FP_LEFT_SWIPE && down){
            performKeyAction(mFpLeftSwipe, event);
        }
